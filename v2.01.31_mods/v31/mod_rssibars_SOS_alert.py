...
##And more useful flashlight options
## Courtesy of LarSeN
##RSSI level calculation is :
##  [VHF] @14B2 origin offset, @14B4 slope, @14B6 s=0 limit.
##  [UHF]: @14CE origin offset, @14D0 slope, @14D2 s=0 limit.
##NEED to change Symbols font: 7 bars w/o antenna icon (Symbols_mods.bmp, is already default)
#Some spare bytes are used for new functions like SOS flashlight while Alarm triggered, and visible blinking flash alert when SelCall rings

#Here you can customize morse patterns for flashlight, when Blinking or Alarm. 0 is On, 1 Off. Read from right to left and DON'T change length :)
#Classic SOS signal
panic_pat = "11111110101011000100010001101010"
#Standard blinking
blink_pat = "10101010101010101010101010101010"
...
if fw[0x855E:0x855E+8] == b'\x43\xD0\x05\x22\x26\x49\x20\x46':
    ...
    fw[0x92F4:0x92F4+16] = b'\x66\x49\x03\x22\x17\x39\x72\x43\x20\x46\xf6\xf7\x4F\xff\x30\xE0'    #in disp refresh routine
    ##fw[0x9304:0x9366] = b'\x00'*98    #Freed bytes to 00
    print('Adding new morse code routine for flashlight...')
    # Kill old blinker management
    fw[0x45FE:0x45FE+8] = b'\x00\xBF\x00\xBF\x00\xBF\x00\xBF'
    fw[0x466E:0x466E+14] = b'\x04\xF0\x49\xFE\x10\x80\x00\xBF\x00\xBF\x00\xBF\x00\xBF'
    # Insert new routine
    ##fw[0x9304:0x9304+98] = b''
    fw[0x9304:0x9304+98] = b''+bitstring_to_bytes(panic_pat)+bitstring_to_bytes(blink_pat)+b'\x00\x10\x06\x40\x00\x02\x00\x00\x00\x00'
