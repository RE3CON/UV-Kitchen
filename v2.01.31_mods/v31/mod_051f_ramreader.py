shellcode = b''

##--------------------- do not modify below this line ---------------------------------------------------
import os,sys,struct

print('Running',os.path.basename(sys.argv[0]),'mod...')
fw =  bytearray(open(sys.argv[1],'rb').read())

fw[0x0D2C:0x0E20] = b'\x00'*len(fw[0x0D04:0x0DF8])   # pour v31, non teste! Apparemment l'adr RAM UART_CMD_BUFFER (0x20000584) n'a aucune garantie d'etre la meme.
fw[0x0D2C:0x0D2C+len(shellcode)] = shellcode

open(sys.argv[1],'wb').write(fw)

