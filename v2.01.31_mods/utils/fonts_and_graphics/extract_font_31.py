...
fw = open('k5_v2.01.31_publish_dec_cut.bin','rb').read()

#Misc symbols @0xD3BC
with open('01_08px_symbols.bmp','wb') as f:
    data = fw[0xD3BC:0xD3BC+442]
    f.write(bmp(data,8,len(data)))

#Small digits @0xD694
with open('04_8px_digits.bmp','wb') as f:
    data = fw[0xD694:0xD694+84]
    f.write(bmp(data,8,len(data)))

#Big digits @0xD576
with open('02_16px_digits.bmp','wb') as f:
    data = fw[0xD576:0xD576+286]
    f.write(bmp(data,8,len(data)))

with open('03_16px_digits_stitched.bmp','wb') as f:
    data = mem16toBmp(fw[0xD576:0xD576+286],13)
    f.write(bmp(data,16,len(data)//2))

#Full alphabet @0xD6E1
with open('05_16px_alphabet.bmp','wb') as f:
    data = fw[0xD6E1:0xD6E1+ 16*95] # 95 characters, 16 bytes each
    f.write(bmp(data,8,len(data)))

with open('06_16px_alphabet_stitched.bmp','wb') as f:
    data = mem16toBmp(fw[0xD6E1:0xD6E1+ 16*95],8)
    f.write(bmp(data,16,len(data)//2))

#All of above in one
with open('07_complete.bmp','wb') as f:
    data = fw[0xD3BC:0xD6E1+ 16*95] # 95 characters, 16 bytes each
    f.write(bmp(data,8,len(data)))
